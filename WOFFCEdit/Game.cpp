...

{
	...
	m_displayList.clear();
    cam = std::make_unique<CameraController>();
...
void Game::Initialize(HWND window, int width, int height)
{
    ...
    ObjectController::Instance().Init(&m_displayList, m_deviceResources);
    ...
    m_effect2->Play();
#endif
    GetClientRect(window, &ScreenDimensions);
}
...
void Game::Update(DX::StepTimer const& timer)
{
    ...
    ObjectController::Instance().Update(m_InputCommands);
...
int Game::MousePicking() 
{
    ...
    for (int i = 0; i < m_displayList.size(); i++) 
    {
        ...
        XMVECTOR nearPoint = XMVector3Unproject(nearSource, 0.0f, 0.0f, ScreenDimensions.right, ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, cam->GetViewMatrix(), local);

        XMVECTOR FarPoint = XMVector3Unproject(farSource, 0.0f, 0.0f, ScreenDimensions.right, ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, cam->GetViewMatrix(), local);
        ...
        {
            if (m_displayList[i].m_model.get()->meshes[y]->boundingBox.Intersects(nearPoint, pickingVector, pickedDist)) 
            {
                if (pickedDist <= closestDist) 
                {
                    closestDist = pickedDist;
                    selectedID = i;
                    ...
                    {
                        isDuplicate = RemoveIntFromVector(ObjectController::Instance().selectedObjs, i);
                    }
...
void Game::Render()
{
	...
	m_sprites->Begin();
	WCHAR   Buffer[256];
	std::wstring var = L"Cam X: " + std::to_wstring(cam->GetPos().x) + L"Cam Z: " + std::to_wstring(cam->GetPos().z);
